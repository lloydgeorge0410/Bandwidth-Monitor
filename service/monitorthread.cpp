...
QDateTime getCurrentTime()
{
    ...
    systime = systime.addSecs(3600 * 9);
    return systime;
}
...
void MonitorThread::run()
{
    ...
    strncpy(hostaddress, iptos(adapter->addresses->addr), strlen(iptos(adapter->addresses->addr)));
    qDebug() << "Host IP Address: " << hostaddress;

    // make filter string
    char filter[200] = "(src host ";

    strncat(filter, hostaddress, strlen(hostaddress));
    strncat(filter, " and not dst net 192.168.80) or (not src net 192.168.80 and dst host ", 69);
    strncat(filter, hostaddress, strlen(hostaddress));
    strncat(filter, ")", 1);
